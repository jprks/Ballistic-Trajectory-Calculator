...
initial_velocity_x = initial_velocity_vector_magnitude*cos(deg2rad(initial_angle));
initial_velocity_y = initial_velocity_vector_magnitude*sin(deg2rad(initial_angle));
...
while initial_height >= 0
    ...
    final_distance_x(i) = initial_distance_x(i) + initial_velocity_x*dt;
...
final_angle = rad2deg(atan(final_velocity_y(i-1)/final_velocity_x));
final_velocity_vector_magnitude = sqrt(final_velocity_x^2 + final_velocity_y(i-1)^2);
...
result = {
    ...
    'Final Horizontal Velocity',final_velocity_x,velocity_unit;
